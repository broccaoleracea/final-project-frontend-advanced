...
const UpdateAlat = ({ params }: { params: { id: string } }) => {
  ...
  useEffect(() => {
    if (alatDetailResponse && alatDetailResponse.data) {
      ...
      // Validasi alat_kategori_id agar tidak undefined
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect ke halaman daftar alat dengan query param status=success
      setTimeout(() => {
        router.push("/admin/alat/view?status=success"); // Tambahkan query param
      }, 1000); // Tunggu 1 detik sebelum redirect (opsional)
    } catch (err: any) {
