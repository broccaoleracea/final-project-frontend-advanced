...
const TambahPelanggan = () => {
  ...
  console.log("FormData for additional data:", formDataWithFile); // Debugging: Cek FormData tambahan
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Kirim data tambahan
      const pelangganDataResponse = await addPelangganData(
        formDataWithFile
      ).unwrap();
      ...
      if (err?.status === 400 || err?.status === 500) {
        setError(
          err?.data?.message || "Terjadi kesalahan saat menambahkan pelanggan."
        );
      } else {
