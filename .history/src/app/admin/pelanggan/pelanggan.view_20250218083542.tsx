...
const PelangganView = () => {
  ...
  const handleDelete = async (pelanggan_id: number) => {
    try {
      console.log("Menghapus alat dengan ID:", pelanggan_id);
      await deletePelanggan(pelanggan_id).unwrap(); // Panggil mutation untuk menghapus alat
      await refetchPelanggan(); // Paksa refetch data alat untuk memastikan data terbaru
